...
try {
  ...
  await act('confirm.code', () => click('confirm.cancel'))
  await page.waitForFunction(() => window.showme.state().target === 'settings.limits.international', null, { timeout: 8000 })
  assert.equal(await page.textContent('.showme-caption .step'), 'Step 5 of 7', 'Cancel closes the dialog, so the guide goes back to the checkbox')
  assert.equal(await page.isChecked(sel('settings.limits.international')), false)
  await act('settings.limits.international', () => page.click('label:has([data-guide="settings.limits.international"])'))
  ...
  assert.equal(r.detours.length, 4, 'two wrong clicks, two rewinds')
  assert.equal(r.detours[1].rewound_to, 1)
  assert.match(r.detours[2].clicked, /confirm\.cancel/)
  assert.equal(r.detours[3].rewound_to, 5)
